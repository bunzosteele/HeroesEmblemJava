...
public class TutorialUnitFourthScreen extends MenuScreen
{
	Sprite buttonSprite;
	int xOffset;
	int yOffset;
	...
	private void drawContent(){
		...
		final AtlasRegion region = UnitRenderer.UnitSheets.get(UnitType.Archer).findRegion("Idle-0-" + idleFrame);
		...
		this.game.font.draw(this.game.batcher, "    PowerShot: Hit all units in a straight line, including allies. Can be blocked by terrain.\n    Snipe: Hit any enemy on the battlefield for massive damage with high crit chance. Once per battle.", this.xOffset * 3 / 2, this.yOffset * 3 - 5 * this.game.font.getData().lineHeight, this.xOffset * 5 / 2, -1, true);
	}
