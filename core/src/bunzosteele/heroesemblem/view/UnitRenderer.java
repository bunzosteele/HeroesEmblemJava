...
public final class UnitRenderer
{
	...
	public static void DrawEnemyStats(final HeroesEmblem game, List<List<Tile>> battlefield, final Unit unit, final int initialX, final int initialY, final int scaledSize, boolean hasSpies) throws IOException
	{
		...
		if(hasSpies){
			...
			if(battlefield != null && (unit.y >= 0 && unit.x >=0)){
				...
				game.font.draw(game.batcher, "DEF: " + (unit.defense + battlefield.get(unit.y).get(unit.x).defenseModifier), initialX, initialY - (5 * game.font.getData().lineHeight));
				UnitRenderer.SetEvasionFont(unit, null, battlefield, game.font);
				game.font.draw(game.batcher, "EVP: " + (unit.evasion + battlefield.get(unit.y).get(unit.x).evasionModifier), initialX, initialY - (6 * game.font.getData().lineHeight));
			}else{
	...
	public static void DrawOwnedStats(final HeroesEmblem game, List<List<Tile>> battlefield, final Unit unit, final int initialX, final int initialY, final int scaledSize) throws IOException
	{
		...
		if(battlefield != null && (unit.y >= 0 && unit.x >=0)){
			UnitRenderer.SetDefenseFont(unit, null, battlefield, game.font);
			game.font.draw(game.batcher, "DEF: " + (unit.defense + battlefield.get(unit.y).get(unit.x).defenseModifier), initialX, initialY - (6 * game.font.getData().lineHeight));
			UnitRenderer.SetEvasionFont(unit, null, battlefield, game.font);
			game.font.draw(game.batcher, "EVP: " + (unit.evasion + battlefield.get(unit.y).get(unit.x).evasionModifier), initialX, initialY - (7 * game.font.getData().lineHeight));
		}else{
	...
	public static void DrawOwnedInfo(final HeroesEmblem game, List<List<Tile>> battlefield, final Unit unit, final int initialX, final int initialY, final int scaledSize, final int totalWidth, int xOffset)
	{
		int columnWidth = (totalWidth - (initialX - xOffset)) / 3;
		game.font.setColor(Color.WHITE);
		game.font.getData().setScale(.25f);
		game.font.draw(game.batcher, unit.name, initialX, initialY, columnWidth, 1, false);
		game.font.draw(game.batcher, "HP: " + unit.currentHealth + "/" + unit.maximumHealth + " (" + unit.initialHealth + ")", initialX + 2 * columnWidth, initialY - 2 * game.font.getData().lineHeight, columnWidth, 1, false);
		game.font.draw(game.batcher, "ATK: " + unit.attack + " (" + unit.initialAttack + ")", initialX + columnWidth, initialY, columnWidth, 1, false);
		game.font.draw(game.batcher, "DEF: " + unit.defense + " (" + unit.initialDefense + ")", initialX + columnWidth, initialY - game.font.getData().lineHeight, columnWidth, 1, false);
		game.font.draw(game.batcher, "EVP: " + unit.evasion + " (" + unit.initialEvasion + ")", initialX + columnWidth, initialY - (2 * game.font.getData().lineHeight), columnWidth, 1, false);
		game.font.draw(game.batcher, "ACC: " + unit.accuracy + " (" + unit.initialAccuracy + ")", initialX + 2 * columnWidth, initialY, columnWidth, 1, false);
		game.font.draw(game.batcher, "MOVE: " + unit.movement+ " (" + unit.initialMovement + ")", initialX + 2 * columnWidth, initialY - game.font.getData().lineHeight, columnWidth, 1, false);
		...
		game.font.setColor(Color.WHITE);
		game.font.draw(game.batcher, "LVL: " + unit.level, initialX - columnWidth * 3 / 4, initialY - 4 * game.font.getData().lineHeight, columnWidth, 1, false);
